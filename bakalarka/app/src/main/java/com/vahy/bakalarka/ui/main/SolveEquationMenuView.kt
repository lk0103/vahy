...
import com.vahy.bakalarka.objects.ScaleVariable
import com.vahy.bakalarka.objects.menu.*
import com.vahy.vahy.objects.ScreenObject

class SolveEquationMenuView(context: Context, attrs: AttributeSet)
    : View(context, attrs) {
    ...
    init {
        ...
        screenObjects.add(TaskSolveSolution(context, listOf()))
    }
    ...
    private fun changeSizeScreenObjects() {
        screenObjects.forEach { obj ->
            ...
            } else if (obj is TaskSolveSolution) {
                obj.sizeChanged(
    ...
    private fun touchedTaskSolveEq(event: MotionEvent) =
        screenObjects.filter {
            it is TaskSolveSolution &&
                    it.isIn(event.x.toInt(), event.y.toInt())
        }.map { it as TaskSolveSolution }
    ...
    private fun onTouchMove(event: MotionEvent): Boolean {
        ...
        if (event.x <= tolerance || event.x >= widthView - tolerance ||
            ...
            screenObjects.filter { it is TaskSolveSolution }
                .forEach { (it as TaskSolveSolution).onTouchUp() }
            invalidate()
            return true
        }
    ...
    fun setMapSolutions(variables : List<ScaleVariable>){
        screenObjects.filter { it is TaskSolveSolution }
            .forEach { (it as TaskSolveSolution).setVariables(variables)}
        invalidate()
    }

    fun getUserSolutions() : Map<String, Int> =
        (screenObjects.filter { it is TaskSolveSolution }
            .first() as TaskSolveSolution).getUserSolutions()
}